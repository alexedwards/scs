...
func TestFind(t *testing.T) {
	dsn := os.Getenv("SCS_COCKROACHDB_TEST_DSN")
...
func TestExpiry(t *testing.T) {
	...
	err = p.Commit("session_token", []byte("encoded_data"), time.Now().Add(1*time.Second))
	if err != nil {
		t.Fatal(err)
	}
...
func TestCleanup(t *testing.T) {
	...
	p := NewWithCleanupInterval(db, 2*time.Second)
	defer p.StopCleanup()
