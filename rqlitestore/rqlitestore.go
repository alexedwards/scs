...
func (r *RqliteStore) Find(token string) (b []byte, exists bool, err error) {
	row, err := r.conn.QueryOnePrepared(
		&gorqlite.PreparedStatement{
			Query:     "SELECT data FROM sessions WHERE token = ? AND julianday('now') < expiry",
...
func (r *RqliteStore) Commit(token string, b []byte, expiry time.Time) error {
	_, err := r.conn.WriteOnePrepared(
		&gorqlite.PreparedStatement{
			Query:     "REPLACE INTO sessions (token, data, expiry) VALUES (?, ?, julianday(?))",
			Arguments: []interface{}{token, hex.EncodeToString(b), expiry.UTC().Format("2006-01-02T15:04:05.000")},
		},
	)
	if err != nil {
		return err
