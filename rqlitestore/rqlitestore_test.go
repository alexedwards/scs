...
func TestFind(t *testing.T) {
	...
	_, err = conn.WriteOnePrepared(
		&gorqlite.PreparedStatement{
			Query:     "INSERT INTO sessions VALUES(?, ?, datetime(current_timestamp, '+1 minute'))",
			Arguments: []interface{}{"session_token", hex.EncodeToString([]byte("encoded_data"))},
		},
	)
	if err != nil {
		t.Fatal(err)
...
func TestAll(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		...
		_, err = conn.WriteOnePrepared(
			&gorqlite.PreparedStatement{
				Query:     "INSERT INTO sessions VALUES(?, ?, datetime(current_timestamp, '+1 minute'))",
				Arguments: []interface{}{key, hex.EncodeToString(val)},
			},
		)
		if err != nil {
			t.Fatal(err)
